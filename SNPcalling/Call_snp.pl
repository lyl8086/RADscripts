#!/bin/env perl
## use model from stacks to call snp from pileup files
...
while (<$in_fh>) {
	...
	for (my $i=3; $i<=$#parts; $i+=3) {
		...
		if ($total == 0) {
			...
			# ln L(1/1) = ln(n! / n_1!n_2!n_3!n_4!) + n_1 * ln(n_1 / n) + 
            #   		 (n - n_1) * ln(n - n_1 / 3n)
...
sub basecount {
	...
	if ($base =~ /[\-\+\>\<]|^$|\*/) {
		...
		$base =~ s/\^.|\$//g; # Remove the start or end symbol, also the character followed by ^.
